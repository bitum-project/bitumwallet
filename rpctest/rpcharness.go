...
import (
	...
	rpc "github.com/bitum-project/bitumd/rpcclient"
)
...
// Harness fully encapsulates an active bitumd process, along with an embedded
// bitumwallet to provide a unified platform for creating RPC-driven integration
// tests involving bitumd. The active bitumd node will typically be run in simnet
...
// spends. The active bitumd process is fully managed by Harness, which handles
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	...
	// Start the bitumd node itself. This spawns a new process which will be
	// managed
	...
	// Start bitumwallet. This spawns a new process which will be managed
	...
	// Get a new address from the wallet to be set with bitumd's --miningaddr
	time.Sleep(5 * time.Second)
	var miningAddr bitumutil.Address
	for i := 0; i < 100; i++ {
...
// bitumd process belonging to this Harness instance. If the initial connection
