...
func TestMain(m *testing.M) {
	...
	ntfnHandlersNode := bitumrpcclient.NotificationHandlers{
...
func testValidateAddress(r *Harness, t *testing.T) {
	...
	for _, acct := range accounts {
		...
		// Decode address
		_, err = bitumutil.DecodeAddress(addrStr)
		if err != nil {
			t.Fatalf("Unable to decode address %s: %v", addr.String(), err)
		}

		// Try to validate an address that is not owned by wallet
		otherAddress, err := bitumutil.DecodeAddress("SsqvxBX8MZC5iiKCgBscwt69jg4u4hHhDKU")
		if err != nil {
	...
	DevAddr, err := bitumutil.DecodeAddress(devSubAddrStr)
	if err != nil {
...
func testWalletPassphrase(r *Harness, t *testing.T) {
	...
	if err != nil && err.(*bitumjson.RPCError).Code !=
		bitumjson.ErrRPCWalletPassphraseIncorrect {
		// bitumjson.ErrWalletPassphraseIncorrect.Code
	...
	// bitumjson.ErrRPCWalletUnlockNeeded
	if !strings.HasPrefix(err.Error(),
		strconv.Itoa(int(bitumjson.ErrRPCWalletUnlockNeeded))) {
		t.Fatalf("createnewaccount returned error (%v) instead of %v",
			err, bitumjson.ErrRPCWalletUnlockNeeded)
	}
	...
	if err != nil && err.(*bitumjson.RPCError).Code !=
		bitumjson.ErrRPCWalletAlreadyUnlocked {
		t.Fatalf("WalletPassphrase failed: %v", err)
	}
...
func testGetBalance(r *Harness, t *testing.T) {
	...
	preAccountBalances := make(map[string]bitumjson.GetAccountBalanceResult)
	...
	sendAmount := bitumutil.Amount(700000000)
	...
	postAccountBalances := make(map[string]bitumjson.GetAccountBalanceResult)
...
func testListUnspent(r *Harness, t *testing.T) {
	...
	amountToSend := bitumutil.Amount(700000000)
	...
	// Get *bitumutil.Tx of send to check the inputs
	...
	for _, txIn := range rawTx.MsgTx().TxIn {
		...
		txInIDs[prevOut.String()] = bitumutil.Amount(txIn.ValueIn).ToCoin()
	}
	...
	for txinID, amt := range txInIDs {
		if _, ok := utxosBeforeSend[txinID]; !ok {
			t.Fatalf("Failed to find txid %v (%v BITUM) in list of UTXOs",
				txinID, amt)
		}
	}
...
func testSendFrom(r *Harness, t *testing.T) {
	...
	amountToSend := bitumutil.Amount(1000000)
	...
	fee := bitumutil.Amount(totalSpent - totalSent)
...
func testSendMany(r *Harness, t *testing.T) {
	...
	amountsToSend := []bitumutil.Amount{700000000, 1400000000}
	addresses := []bitumutil.Address{}
	...
	addressAmounts := make(map[bitumutil.Address]bitumutil.Amount)
	totalAmountToSend := bitumutil.Amount(0)
...
func testListTransactions(r *Harness, t *testing.T) {
	...
	if *txList1[0].TxType != bitumjson.LTTTRegular {
	...
	voutAmt := bitumutil.Amount(txOut.Value).ToCoin()
	...
	sendAmount := bitumutil.Amount(240000000)
	...
	var sendResult, recvResult bitumjson.ListTransactionsResult
	...
	rxtxResults := map[string]bitumjson.ListTransactionsResult{
...
func testGetSetRelayFee(r *Harness, t *testing.T) {
	// bitumrpcclient does not have a getwalletfee or any direct method, so we
	...
	origTxFee, err := bitumutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	// Increase fee by 50%
	newTxFeeCoin := walletInfo.TxFee * 1.5
	newTxFee, err := bitumutil.NewAmount(newTxFeeCoin)
	if err != nil {
	...
	newTxFeeActual, err := bitumutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	...
	err = wcl.SetTxFee(bitumutil.Amount(-1))
	if err == nil {
...
func testGetSetTicketFee(r *Harness, t *testing.T) {
	// bitumrpcclient does not have a getticketee or any direct method, so we
	...
	origTicketFee, err := bitumutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	newTicketFee, err := bitumutil.NewAmount(newTicketFeeCoin)
	if err != nil {
	...
	nominalTicketFee = walletInfo.TicketFee
	newTicketFeeActual, err := bitumutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	priceLimit, err := bitumutil.NewAmount(2 * mustGetStakeDiffNext(r, t))
	if err != nil {
	...
	err = wcl.SetTicketFee(bitumutil.Amount(-1))
	if err == nil {
...
func testPurchaseTickets(r *Harness, t *testing.T) {
	...
	origTicketFee, err := bitumutil.NewAmount(walletInfo.TicketFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
	newTicketFee, err := bitumutil.NewAmount(walletInfo.TicketFee * 1.5)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
...
func mustGetStakeInfo(wcl *bitumrpcclient.Client, t *testing.T) *bitumjson.GetStakeInfoResult {
	stakeinfo, err := wcl.GetStakeInfo()
	if err != nil {
		t.Fatal("GetStakeInfo failed: ", err)
	}
...
func newBlockAt(currentHeight uint32, r *Harness,
	t *testing.T) (uint32, *bitumutil.Block, []*chainhash.Hash) {
...
func newBlockAtQuick(currentHeight uint32, r *Harness,
	...
	return block.Header.Height, bitumutil.NewBlock(block), blockHashes
}

func getBestBlock(r *Harness, t *testing.T) (uint32, *bitumutil.Block, *chainhash.Hash) {
	...
	return curBlockHeight, bitumutil.NewBlock(bestBlock), bestBlockHash
}
...
func includesTx(txHash *chainhash.Hash, block *bitumutil.Block) bool {
	if len(block.Transactions()) <= 1 {
		return false
	}
...
// includesTx checks if a block contains a transaction hash
func includesStakeTx(txHash *chainhash.Hash, block *bitumutil.Block) bool {
...
// spent minus sent.
func getWireMsgTxFee(tx *bitumutil.Tx) bitumutil.Amount {
	var totalSpent int64
	...
	return bitumutil.Amount(totalSpent - totalSent)
}

// getOutPointString uses OutPoint.String() to combine the tx hash with vout
// index from a ListUnspentResult.
func getOutPointString(utxo *bitumjson.ListUnspentResult) (string, error) {
