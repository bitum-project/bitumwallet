...
var (
	walletDataDirectory = bitumutil.AppDataDir("bitumwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet               bool    `long:"testnet" description:"Use the test bitum network"`
	SimNet                bool    `long:"simnet" description:"Use the simulation bitum network"`
...
func init() {
	...
	if opts.TestNet && opts.SimNet {
		fatalf("Multiple bitum networks may not be used simultaneously")
	}
...
func makeInputSource(outputs []bitumjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue   bitumutil.Amount
		inputs            = make([]*wire.TxIn, 0, len(outputs))
		redeemScriptSizes = make([]int, 0, len(outputs))
		sourceErr         error
	)
	for _, output := range outputs {
		outputAmount, err := bitumutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(bitumutil.Amount) (*txauthor.InputDetail, error) {
...
type destinationScriptSourceToAccount struct {
	accountName string
	rpcClient   *bitumrpcclient.Client
}
...
func (src *destinationScriptSourceToAddress) Script() ([]byte, uint16, error) {
	destinationAddress, err := bitumutil.DecodeAddress(src.address)
	if err != nil {
		return nil, 0, err
	}
	script, err := txscript.PayToAddrScript(destinationAddress)
...
func sweep() error {
	...
	rpcClient, err := bitumrpcclient.New(&bitumrpcclient.ConnConfig{
	...
	sourceOutputs := make(map[string][]bitumjson.ListUnspentResult)
	...
	var totalSwept bitumutil.Amount
	var numErrors int
	...
	feeRate, err := bitumutil.NewAmount(opts.FeeRate)
	if err != nil {
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := bitumutil.Amount(atx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount bitumutil.Amount) bool {
	return amount >= 0 && amount <= bitumutil.MaxAmount
}

func parseOutPoint(input *bitumjson.ListUnspentResult) (wire.OutPoint, error) {
