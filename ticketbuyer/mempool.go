...
func (t *TicketPurchaser) ownTicketsInMempool() (int, error) {
	...
	if t.votingAddress != nil {
		tiHashes, err := t.bitumdChainSvr.GetRawMempool(bitumjson.GRMTickets)
		if err != nil {
			return 0, err
		}
		...
		for i := range tiHashes {
			raw, err := t.bitumdChainSvr.GetRawTransactionVerbose(tiHashes[i])
			if err != nil {
				return 0, err
			}
...
func (t *TicketPurchaser) allTicketsInMempool() (int, error) {
	tfi, err := t.bitumdChainSvr.TicketFeeInfo(&zeroUint32, &zeroUint32)
	if err != nil {
		return 0, err
	}
