...
type TicketPurchaser struct {
	...
	bitumdChainSvr     *bitumrpcclient.Client
	...
	ticketPrice      bitumutil.Amount
	stakePoolSize    uint32
	stakeLive        uint32
	stakeImmature    uint32
	stakeVoteSubsidy bitumutil.Amount
	...
	maxPriceAbsolute  bitumutil.Amount
	maxPriceRelative  float64
	maxFee            bitumutil.Amount
	minFee            bitumutil.Amount
	poolFees          float64
	maxPerBlock       int
	maxInMempool      int
	expiryDelta       int
...
func (t *TicketPurchaser) BalanceToMaintain() bitumutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetBalanceToMaintain(balanceToMaintain int64) {
	t.purchaserMtx.Lock()
	t.balanceToMaintain = bitumutil.Amount(balanceToMaintain)
	t.purchaserMtx.Unlock()
}

// MaxPriceAbsolute returns the max absolute price to purchase a ticket.
func (t *TicketPurchaser) MaxPriceAbsolute() bitumutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetMaxPriceAbsolute(maxPriceAbsolute int64) {
	t.purchaserMtx.Lock()
	t.maxPriceAbsolute = bitumutil.Amount(maxPriceAbsolute)
	t.purchaserMtx.Unlock()
}
...
func (t *TicketPurchaser) MaxFee() bitumutil.Amount {
	t.purchaserMtx.Lock()
	maxFee := t.maxFee
	t.purchaserMtx.Unlock()
	return maxFee
...
func (t *TicketPurchaser) SetMaxFee(maxFee int64) {
	t.purchaserMtx.Lock()
	t.maxFee = bitumutil.Amount(maxFee)
	t.purchaserMtx.Unlock()
}

// MinFee returns the min ticket fee per KB to use when purchasing tickets.
func (t *TicketPurchaser) MinFee() bitumutil.Amount {
	t.purchaserMtx.Lock()
	minFee := t.minFee
	t.purchaserMtx.Unlock()
	return minFee
...
func (t *TicketPurchaser) SetMinFee(minFee int64) {
	t.purchaserMtx.Lock()
	t.minFee = bitumutil.Amount(minFee)
	t.purchaserMtx.Unlock()
}

// VotingAddress returns the address to send ticket outputs to.
func (t *TicketPurchaser) VotingAddress() bitumutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetVotingAddress(votingAddress bitumutil.Address) {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) PoolAddress() bitumutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetPoolAddress(poolAddress bitumutil.Address) {
	t.purchaserMtx.Lock()
...
func NewTicketPurchaser(cfg *Config,
	...
	return &TicketPurchaser{
		...
		account:           account,
		balanceToMaintain: bitumutil.Amount(cfg.BalanceToMaintainAbsolute),
		maxFee:            bitumutil.Amount(cfg.MaxFee),
		minFee:            bitumutil.Amount(cfg.MinFee),
		maxPerBlock:       cfg.MaxPerBlock,
		maxPriceAbsolute:  bitumutil.Amount(cfg.MaxPriceAbsolute),
...
type PurchaseStats struct {
	...
	PriceAverage  bitumutil.Amount
	PriceNext     bitumutil.Amount
	PriceCurrent  bitumutil.Amount
	Purchased     int
	LeftWindow    int
	Balance       bitumutil.Amount
	TicketPrice   bitumutil.Amount
}
...
func (t *TicketPurchaser) Purchase(height int64) (*PurchaseStats, error) {
	...
	if t.firstStart {
		...
		log.Debugf("Transaction relay fee: %v", bitumutil.Amount(t.cfg.TxFee))
	...
	if refreshStakeInfo {
		...
		for i := 1; i <= stakeInfoReqTries; i++ {
			curStakeInfo, err = t.wallet.StakeInfoPrecise(t.bitumdChainSvr)
			if err != nil {
		...
		t.stakeVoteSubsidy = bitumutil.Amount(subsidy)
	...
	if avgPriceAmt < bitumutil.Amount(t.activeNet.MinimumStakeDiff) {
		avgPriceAmt = bitumutil.Amount(t.activeNet.MinimumStakeDiff)
	}
	...
	if err != nil && t.bitumdChainSvr != nil {
		// Wallet failed to calculate sdiff (DCP0001 may not be active), so
		// query it over RPC.
		var sd *bitumjson.GetStakeDifficultyResult
		sd, err = t.bitumdChainSvr.GetStakeDifficulty()
		if err == nil {
			nextStakeDiff, err = bitumutil.NewAmount(sd.NextStakeDifficulty)
			if err != nil {
				return ps, err
			}
		}
	...
	sDiffEsts, err := t.bitumdChainSvr.EstimateStakeDiff(nil)
	if err == nil {
		ps.PriceNext, err = bitumutil.NewAmount(sDiffEsts.Expected)
		if err != nil {
			return ps, err
		}
	...
	var maxPriceAmt bitumutil.Amount
	...
	oneBlock := uint32(1)
	info, err := t.bitumdChainSvr.TicketFeeInfo(&oneBlock, &zeroUint32)
	if err != nil {
		return ps, err
	}
	...
	if !t.cfg.NoSpreadTicketPurchases && toBuyForBlock > 0 {
		...
		log.Debugf("Your average purchase price for tickets in the pool is %.2f BITUM", yourAvgTixPrice)
		log.Debugf("Available funds of %.2f BITUM can buy %.2f tickets, %.2f tickets per block",
		...
		log.Debugf("Redeemed ticket value expected is %.2f BITUM, buys %.2f tickets, %.2f%% more",
			redeemedFunds, tixToBuyWithRedeemedFunds, tixToBuyWithRedeemedFunds/tixCanBuy*100)
		log.Debugf("Stake reward expected is %.2f BITUM, buys %.2f tickets, %.2f%% more",
	...
	// have enough moneys.
	notEnough := func(bal bitumutil.Amount, toBuy int, sd bitumutil.Amount) bool {
	...
	// Purchase tickets.
	poolFeesAmt, err := bitumutil.NewAmount(t.cfg.PoolFees)
	if err != nil {
		return ps, err
	}
