...
type diffPeriodFee struct {
	difficulty bitumutil.Amount
	difference bitumutil.Amount // Difference from current difficulty
...
func (t *TicketPurchaser) findClosestFeeWindows(difficulty bitumutil.Amount,
	useMedian bool) (bitumutil.Amount, error) {
	wtcUint32 := uint32(windowsToConsider)
	info, err := t.bitumdChainSvr.TicketFeeInfo(&zeroUint32, &wtcUint32)
	if err != nil {
		return 0.0, err
	}
	...
	for i := range info.FeeInfoWindows {
		...
		blH, err := t.bitumdChainSvr.GetBlockHash(startHeight)
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.bitumdChainSvr.GetBlockHeader(blH)
		if err != nil {
			return 0, err
		}

		windowDiffAmt := bitumutil.Amount(blkHeader.SBits)

		var fee bitumutil.Amount
		if !useMedian {
			fee, err = bitumutil.NewAmount(info.FeeInfoWindows[i].Mean)
		} else {
			fee, err = bitumutil.NewAmount(info.FeeInfoWindows[i].Median)
		}
		if err != nil {
			return 0, err
		}
...
func (t *TicketPurchaser) findTicketFeeBlocks(useMedian bool) (bitumutil.Amount, error) {
	btaUint32 := uint32(t.cfg.BlocksToAvg)
	info, err := t.bitumdChainSvr.TicketFeeInfo(&btaUint32, nil)
	if err != nil {
		return 0.0, err
	}

	var sum, tmp bitumutil.Amount
	for i := range info.FeeInfoBlocks {
		if !useMedian {
			tmp, err = bitumutil.NewAmount(info.FeeInfoBlocks[i].Mean)
		} else {
			tmp, err = bitumutil.NewAmount(info.FeeInfoBlocks[i].Median)
		}
		if err != nil {
			return 0, err
		}
	...
	return sum / bitumutil.Amount(t.cfg.BlocksToAvg), nil
}
