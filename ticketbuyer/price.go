...
func (t *TicketPurchaser) calcAverageTicketPrice(height int64) (bitumutil.Amount, error) {
	...
	// these two prices.
	var avgPricePoolAmt bitumutil.Amount
	if t.priceMode == AvgPricePoolMode || t.priceMode == AvgPriceDualMode {
		poolValue, err := t.bitumdChainSvr.GetTicketPoolValue()
		if err != nil {
			return 0, err
		}
		bestBlockH, err := t.bitumdChainSvr.GetBestBlockHash()
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.bitumdChainSvr.GetBlockHeader(bestBlockH)
		if err != nil {
			return 0, err
		}
		...
		avgPricePoolAmt = poolValue / bitumutil.Amount(poolSize)
	...
	var ticketVWAP bitumutil.Amount
	if t.priceMode == AvgPriceVWAPMode || t.priceMode == AvgPriceDualMode {
		...
		for i := 0; i < vwapReqTries; i++ {
			ticketVWAP, err = t.bitumdChainSvr.TicketVWAP(&startVWAPHeight,
				&endVWAPHeight)
