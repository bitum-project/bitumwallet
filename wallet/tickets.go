...
func (w *Wallet) LiveTicketHashes(chainClient *bitumrpcclient.Client, includeImmature bool) ([]chainhash.Hash, error) {
	...
	// these transactions are not part of the wallet's transaction history, bitumd
	// must be queried for their blockchain height.  This functionality requires
	// the bitumd transaction index to be enabled.
...
// pruned tickets.
func (w *Wallet) TicketHashesForVotingAddress(votingAddr bitumutil.Address) ([]chainhash.Hash, error) {
...
func (w *Wallet) AddTicket(ticket *wire.MsgTx) error {
	...
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		err := w.StakeMgr.InsertSStx(stakemgrNs, bitumutil.NewTx(ticket))
		if err != nil {
			return err
		}
...
// revocations.
func (w *Wallet) RevokeTickets(chainClient *bitumrpcclient.Client) error {
