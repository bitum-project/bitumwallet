...
func (w *Wallet) StakePoolUserInfo(userAddress bitumutil.Address) (*udb.StakePoolUser, error) {
	const op errors.Op = "wallet.StakePoolUserInfo"

	switch userAddress.(type) {
	case *bitumutil.AddressPubKeyHash: // ok
	case *bitumutil.AddressScriptHash: // ok
	default:
