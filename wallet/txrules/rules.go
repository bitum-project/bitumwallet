...
const DefaultRelayFeePerKb bitumutil.Amount = 1e4

// IsDustAmount determines whether a transaction output value and script length would
// cause the output to be considered dust.  Transactions with dust outputs are
// not standard and are rejected by mempools with default policies.
func IsDustAmount(amount bitumutil.Amount, scriptSize int, relayFeePerKb bitumutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb bitumutil.Amount) bool {
	...
	return IsDustAmount(bitumutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
func CheckOutput(output *wire.TxOut, relayFeePerKb bitumutil.Amount) error {
	if output.Value < 0 {
		return errors.E(errors.Invalid, "transaction output amount is negative")
	}
	if output.Value > bitumutil.MaxAmount {
...
func FeeForSerializeSize(relayFeePerKb bitumutil.Amount, txSerializeSize int) bitumutil.Amount {
	fee := relayFeePerKb * bitumutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > bitumutil.MaxAmount {
		fee = bitumutil.MaxAmount
	}
...
func PaysHighFees(totalInput bitumutil.Amount, tx *wire.MsgTx) bool {
