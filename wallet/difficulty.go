...
// This code was copied from bitumd/blockchain/difficulty.go and modified for
// bitumwallet's header storage.
...
func (w *Wallet) ancestorHeaderAtHeight(dbtx walletdb.ReadTx, h *wire.BlockHeader, chain []*BlockNode, height int32) (*wire.BlockHeader, error) {
	...
	case height > int32(h.Height), height < 0:
		return nil, nil // bitumd's blockNode.Ancestor returns nil for child heights
	}
...
// in DCP0001.
func (w *Wallet) nextRequiredDCP0001PoSDifficulty(dbtx walletdb.ReadTx, curHeader *wire.BlockHeader, chain []*BlockNode) (bitumutil.Amount, error) {
	...
	if nextHeight < stakeDiffStartHeight {
		return bitumutil.Amount(w.chainParams.MinimumStakeDiff), nil
	}
	...
	if nextHeight%intervalSize != 0 {
		return bitumutil.Amount(curDiff), nil
	}
	...
	return bitumutil.Amount(sdiff), nil
}
...
// trusted full node.
func (w *Wallet) NextStakeDifficulty() (bitumutil.Amount, error) {
	const op errors.Op = "wallet.NextStakeDifficulty"
	var sdiff bitumutil.Amount
...
func (w *Wallet) NextStakeDifficultyAfterHeader(h *wire.BlockHeader) (bitumutil.Amount, error) {
