...
func lookupOutputChain(dbtx walletdb.ReadTx, w *Wallet, details *udb.TxDetails,
	cred udb.CreditRecord) (account uint32, internal bool, address bitumutil.Address,
...
func makeTxSummary(dbtx walletdb.ReadTx, w *Wallet, details *udb.TxDetails) TransactionSummary {
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		...
		for _, txOut := range details.MsgTx.TxOut {
			fee -= bitumutil.Amount(txOut.Value)
		}
	}
	...
	for i := range details.MsgTx.TxOut {
		...
		output := TransactionSummaryOutput{
			...
			Account:      acct,
			Internal:     internal,
			Amount:       bitumutil.Amount(amount),
			Address:      address,
...
func makeTicketSummary(chainClient *bitumrpcclient.Client, dbtx walletdb.ReadTx, w *Wallet, details *udb.TicketDetails) *TicketSummary {
...
func totalBalances(dbtx walletdb.ReadTx, w *Wallet, m map[uint32]bitumutil.Amount) error {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
...
func flattenBalanceMap(m map[uint32]bitumutil.Amount) []AccountBalance {
...
func relevantAccounts(w *Wallet, m map[uint32]bitumutil.Amount, txs []TransactionSummary) {
	for _, tx := range txs {
...
func (s *NotificationServer) notifyUnminedTransaction(dbtx walletdb.ReadTx, details *udb.TxDetails) {
	...
	bals := make(map[uint32]bitumutil.Amount)
...
type TransactionSummaryInput struct {
	...
	PreviousAmount  bitumutil.Amount
}
