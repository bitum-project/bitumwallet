...
// account.
func (w *Wallet) NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb bitumutil.Amount, account uint32, minConf int32,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		case OutputSelectionAlgorithmAll:
			...
			inputSource = func(bitumutil.Amount) (*txauthor.InputDetail, error) {
				inputDetail, err := sourceImpl.SelectInputs(bitumutil.MaxAmount)
...
func (s *secretSource) GetKey(addr bitumutil.Address) (chainec.PrivateKey, bool, error) {
...
func (s *secretSource) GetScript(addr bitumutil.Address) ([]byte, error) {
...
func (w *Wallet) checkHighFees(totalInput bitumutil.Amount, tx *wire.MsgTx) error {
	if w.AllowHighFees {
		return nil
	}
...
//
// Bitum: This func also sends the transaction, and if successful, inserts it
...
func (w *Wallet) txToOutputsInternal(op errors.Op, outputs []*wire.TxOut, account uint32, minconf int32,
	n NetworkBackend, randomizeChangeIdx bool, txFee bitumutil.Amount) (*txauthor.AuthoredTx, error) {
	...
	if atx.ChangeIndex >= 0 && account == udb.ImportedAddrAccount {
		changeAmount := bitumutil.Amount(atx.Tx.TxOut[atx.ChangeIndex].Value)
...
func (w *Wallet) txToMultisig(op errors.Op, account uint32, amount bitumutil.Amount, pubkeys []*bitumutil.AddressSecpPubKey,
	nRequired int8, minconf int32) (*CreatedTx, bitumutil.Address, []byte, error) {

	var (
		ctx      *CreatedTx
		addr     bitumutil.Address
		msScript []byte
	)
	err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		var err error
...
func (w *Wallet) txToMultisigInternal(op errors.Op, dbtx walletdb.ReadWriteTx, account uint32, amount bitumutil.Amount,
	pubkeys []*bitumutil.AddressSecpPubKey, nRequired int8, minconf int32) (*CreatedTx, bitumutil.Address, []byte, error) {

	addrmgrNs := dbtx.ReadWriteBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadWriteBucket(wtxmgrNamespaceKey)

	txToMultisigError := func(err error) (*CreatedTx, bitumutil.Address, []byte, error) {
	...
	// fee estimator.
	var feeEstForTx bitumutil.Amount
	...
	scAddr, err := bitumutil.NewAddressScriptHash(msScript, w.chainParams)
	if err != nil {
		return txToMultisigError(errors.E(op, err))
	}
	...
	// Request updates from bitumd for new transactions sent to this
	// script hash address.
	err = n.LoadTxFilter(context.TODO(), false, []bitumutil.Address{scAddr}, nil)
	if err != nil {
		return txToMultisigError(errors.E(op, err))
	}
...
func (w *Wallet) compressWallet(op errors.Op, maxNumIns int, account uint32, changeAddr bitumutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) compressWalletInternal(op errors.Op, dbtx walletdb.ReadWriteTx, maxNumIns int, account uint32,
	changeAddr bitumutil.Address) (*chainhash.Hash, error) {
...
// passed.
func makeTicket(params *chaincfg.Params, inputPool *extendedOutPoint, input *extendedOutPoint, addrVote bitumutil.Address,
	addrSubsidy bitumutil.Address, ticketCost int64, addrPool bitumutil.Address) (*wire.MsgTx, error) {
	...
	zeroed := [20]byte{}
	addrZeroed, err := bitumutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
		return nil, err
	}
	...
	if addrPool != nil {
		pkScript, err = txscript.GenerateSStxAddrPush(addrPool,
			bitumutil.Amount(amountsCommitted[0]), limits)
		if err != nil {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addrSubsidy,
		bitumutil.Amount(amountsCommitted[userSubsidyNullIdx]), limits)
	if err != nil {
		return nil, errors.E(errors.Op("txscript.GenerateSStxAddrPush"), errors.Invalid,
...
func (w *Wallet) purchaseTickets(op errors.Op, req purchaseTicketRequest) ([]*chainhash.Hash, error) {
	...
	if w.addressReuse {
		...
		addrFunc = func(errors.Op, persistReturnedChildFunc, uint32) (bitumutil.Address, error) {
			return addr, err
		}
	}
	...
	case *bitumutil.AddressScriptHash:
		stakeSubmissionPkScriptSize = txsizes.P2SHPkScriptSize + 1
	case *bitumutil.AddressPubKeyHash, nil:
	...
	var neededPerTicket, ticketFee bitumutil.Amount
	var estSize int
	...
	// do so now.
	var poolFeeAmt bitumutil.Amount
	if poolAddress != nil {
	...
	if req.minBalance > 0 {
		...
		estimatedFundsUsed := neededPerTicket * bitumutil.Amount(req.numTickets)
	...
	for i := 0; i < req.numTickets; i++ {
		...
		// an address.
		var addrVote, addrSubsidy bitumutil.Address
		err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		if eopPool != nil {
			eopPoolCredit := udb.Credit{
				...
				Amount:       bitumutil.Amount(eopPool.amt),
		...
		eopCredit := udb.Credit{
			OutPoint:     *eop.op,
			BlockMeta:    udb.BlockMeta{},
			Amount:       bitumutil.Amount(eop.amt),
		...
		err = w.checkHighFees(bitumutil.Amount(eop.amt), ticket)
		if err != nil {
			return ticketHashes, errors.E(op, err)
		}
...
func (w *Wallet) findEligibleOutputsAmount(dbtx walletdb.ReadTx, account uint32, minconf int32,
	amount bitumutil.Amount, currentHeight int32) ([]udb.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)
	var outTotal bitumutil.Amount
...
func (w *Wallet) signVoteOrRevocation(addrmgrNs walletdb.ReadBucket, ticketPurchase, tx *wire.MsgTx, isVote bool) error {
	...
	// can be performed.
	var getKey txscript.KeyClosure = func(addr bitumutil.Address) (chainec.PrivateKey, bool, error) {
		...
		return key, true, nil // secp256k1 pubkeys are always compressed in Bitum
	}
	var getScript txscript.ScriptClosure = func(addr bitumutil.Address) ([]byte, error) {
	...
	signedScript, err := txscript.SignTxOutput(w.chainParams, tx, inputToSign,
		...
		tx.TxIn[inputToSign].SignatureScript, bitumec.STEcdsaSecp256k1)
	if err != nil {
...
func createUnsignedRevocation(ticketHash *chainhash.Hash, ticketPurchase *wire.MsgTx, feePerKB bitumutil.Amount) (*wire.MsgTx, error) {
	// Parse the ticket purchase transaction to determine the required output
	// destinations for vote rewards or revocations.
	ticketPayKinds, ticketHash160s, ticketValues, _, _, _ :=
		stake.TxSStxStakeOutputInfo(ticketPurchase)
	...
	for _, output := range revocation.TxOut {
		if bitumutil.Amount(output.Value) > feeEstimate {
			amount := bitumutil.Amount(output.Value) - feeEstimate
