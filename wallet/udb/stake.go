...
func (s *StakeStore) insertSStx(ns walletdb.ReadWriteBucket, sstx *bitumutil.Tx) error {
...
func (s *StakeStore) dumpSStxHashesForAddress(ns walletdb.ReadBucket, addr bitumutil.Address) ([]chainhash.Hash, error) {
	// Extract the HASH160 script hash; if it's not 20 bytes
	// long, return an error.
	hash160 := addr.ScriptAddress()
	_, addrIsP2SH := addr.(*bitumutil.AddressScriptHash)
...
func (s *StakeStore) sstxAddress(ns walletdb.ReadBucket, hash *chainhash.Hash) (bitumutil.Address, error) {
	// Access the database and store the result locally.
	...
	if p2sh {
		addr, err = bitumutil.NewAddressScriptHashFromHash(thisHash160, s.Params)
	} else {
		addr, err = bitumutil.NewAddressPubKeyHash(thisHash160, s.Params, bitumec.STEcdsaSecp256k1)
	}
	if err != nil {
		return nil, err
	}
...
func (s *StakeStore) updateStakePoolUserTickets(ns walletdb.ReadWriteBucket, user bitumutil.Address, ticket *PoolTicket) error {
	_, isScriptHash := user.(*bitumutil.AddressScriptHash)
	_, isP2PKH := user.(*bitumutil.AddressPubKeyHash)
...
func (s *StakeStore) removeStakePoolUserInvalTickets(ns walletdb.ReadWriteBucket, user bitumutil.Address, ticket *chainhash.Hash) error {
	_, isScriptHash := user.(*bitumutil.AddressScriptHash)
	_, isP2PKH := user.(*bitumutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
		return errors.E(errors.Invalid, errors.Errorf("voting address type %T", user))
...
// database, it adds it.
func (s *StakeStore) updateStakePoolUserInvalTickets(ns walletdb.ReadWriteBucket, user bitumutil.Address, ticket *chainhash.Hash) error {
	_, isScriptHash := user.(*bitumutil.AddressScriptHash)
	_, isP2PKH := user.(*bitumutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
		return errors.E(errors.Invalid, errors.Errorf("voting address type %T", user))
...
func stakePoolUserInfo(ns walletdb.ReadBucket, user bitumutil.Address) (*StakePoolUser, error) {
	_, isScriptHash := user.(*bitumutil.AddressScriptHash)
	_, isP2PKH := user.(*bitumutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
...
func (s *StakeStore) StakePoolUserInfo(ns walletdb.ReadBucket, user bitumutil.Address) (*StakePoolUser, error) {
