...
type ManagedAddress interface {
	...
	// Address returns a bitumutil.Address for the backing address.
...
type managedAddress struct {
	manager    *Manager
	account    uint32
	address    *bitumutil.AddressPubKeyHash
	imported   bool
	internal   bool
	multisig   bool
	compressed bool
...
// Address returns the bitumutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() bitumutil.Address {
	return a.address
}
...
func newManagedAddressWithoutPrivKey(m *Manager, account uint32, pubKey chainec.PublicKey, compressed bool) (*managedAddress, error) {
	...
	var pubKeyHash []byte
	if compressed {
		pubKeyHash = bitumutil.Hash160(pubKey.SerializeCompressed())
	} else {
		pubKeyHash = bitumutil.Hash160(pubKey.SerializeUncompressed())
	}
	address, err := bitumutil.NewAddressPubKeyHash(pubKeyHash, m.chainParams,
		bitumec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
...
type scriptAddress struct {
	manager *Manager
	account uint32
	address *bitumutil.AddressScriptHash
}
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() bitumutil.Address {
	return a.address
}
...
func newScriptAddress(m *Manager, account uint32, scriptHash []byte) (*scriptAddress, error) {
	address, err := bitumutil.NewAddressScriptHashFromHash(scriptHash,
		m.chainParams)
	if err != nil {
		return nil, err
	}
