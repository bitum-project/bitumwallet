...
func deserializeSStxRecord(serializedSStxRecord []byte, dbVersion uint32) (*sstxRecord, error) {
	...
	case dbVersion < 3:
		...
		// Create and save the bitumutil.Tx of the read MsgTx and set its index.
		tx := bitumutil.NewTx(msgTx)
		tx.SetIndex(bitumutil.TxIndexUnknown)
	...
	case dbVersion >= 3:
		...
		return &sstxRecord{tx: bitumutil.NewTx(&tx), ts: time.Unix(unixTime, 0)}, nil
