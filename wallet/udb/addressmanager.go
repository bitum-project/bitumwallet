...
func normalizeAddress(addr bitumutil.Address) bitumutil.Address {
...
// now created using the coin types assigned to Bitum in SLIP0044:
//
...
// to the coin type registered to Bitum as per SLIP0044.  On mainnet, this
...
//
// This function MUST be called with the manager lock held for writes.
func (m *Manager) loadAddress(ns walletdb.ReadBucket, address bitumutil.Address) (ManagedAddress, error) {
...
func (m *Manager) Address(ns walletdb.ReadBucket, address bitumutil.Address) (ManagedAddress, error) {
...
func (m *Manager) AddrAccount(ns walletdb.ReadBucket, address bitumutil.Address) (uint32, error) {
...
func (m *Manager) ImportPrivateKey(ns walletdb.ReadWriteBucket, wif *bitumutil.WIF) (ManagedPubKeyAddress, error) {
	defer m.mtx.Unlock()
	m.mtx.Lock()
	...
	pubKeyHash := bitumutil.Hash160(serializedPubKey)
...
func (m *Manager) ImportScript(ns walletdb.ReadWriteBucket, script []byte) (ManagedScriptAddress, error) {
	...
	// Prevent duplicates.
	scriptHash := bitumutil.Hash160(script)
...
func (m *Manager) MarkUsed(ns walletdb.ReadWriteBucket, address bitumutil.Address) error {
...
func (m *Manager) syncAccountToAddrIndex(ns walletdb.ReadWriteBucket, account uint32, syncToIndex uint32, branch uint32) error {
	...
	for child := syncToIndex; ; child-- {
		...
		// bitumutil.NewAddressPubKeyHash.
...
func (m *Manager) ForEachActiveAddress(ns walletdb.ReadBucket, fn func(addr bitumutil.Address) error) error {
	m.mtx.Lock()
	defer m.mtx.Unlock()
...
func (m *Manager) PrivateKey(ns walletdb.ReadBucket, addr bitumutil.Address) (key chainec.PrivateKey, done func(), err error) {
...
func (m *Manager) RedeemScript(ns walletdb.ReadBucket, addr bitumutil.Address) (script []byte, done func(), err error) {
	// Lock the manager mutex for writes.  This protects read access to m.locked
