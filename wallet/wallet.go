...
import (
	...
	"github.com/bitum-project/bitumd/chaincfg/chainec"
...
type Wallet struct {
	...
	votingEnabled      bool
	balanceToMaintain  bitumutil.Amount
	poolAddress        bitumutil.Address
	poolFees           float64
	...
	relayFee               bitumutil.Amount
	relayFeeMu             sync.Mutex
	ticketFeeIncrementLock sync.Mutex
	ticketFeeIncrement     bitumutil.Amount
	DisallowFree           bool
	AllowHighFees          bool
	...
	addressReuse     bool
	ticketAddress    bitumutil.Address
...
type Config struct {
	...
	VotingEnabled bool
	AddressReuse  bool
	VotingAddress bitumutil.Address
	PoolAddress   bitumutil.Address
	PoolFees      float64
	TicketFee     float64
...
func (w *Wallet) BalanceToMaintain() bitumutil.Amount {
...
func (w *Wallet) SetBalanceToMaintain(balance bitumutil.Amount) {
	w.stakeSettingsLock.Lock()
...
func (w *Wallet) RelayFee() bitumutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee bitumutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
func (w *Wallet) TicketFeeIncrement() bitumutil.Amount {
...
func (w *Wallet) SetTicketFeeIncrement(fee bitumutil.Amount) {
	w.ticketFeeIncrementLock.Lock()
...
func (w *Wallet) loadActiveAddrs(ctx context.Context, dbtx walletdb.ReadTx, nb NetworkBackend) (uint64, error) {
	...
	loadBranchAddrs := func(branchKey *hdkeychain.ExtendedKey, n uint32, errs chan<- error) {
		...
		for child := uint32(0); child <= n; child += step {
			child := child
			g.Go(func() error {
				addrs := make([]bitumutil.Address, 0, step)
...
func (w *Wallet) CommittedTickets(tickets []*chainhash.Hash) ([]*chainhash.Hash, []bitumutil.Address, error) {
	const op errors.Op = "wallet.CommittedTickets"
	hashes := make([]*chainhash.Hash, 0, len(tickets))
	addresses := make([]bitumutil.Address, 0, len(tickets))
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for _, v := range tickets {
			...
			var bestAddr bitumutil.Address
			var bestAmount bitumutil.Amount
...
// message or bitumd JSON-RPC request, for the blocks in sidechain and saved in
...
type (
	...
	createMultisigTxRequest struct {
		...
		pubkeys   []*bitumutil.AddressSecpPubKey
		nrequired int8
		minconf   int32
		resp      chan createMultisigTxResponse
	}
	purchaseTicketRequest struct {
		minBalance  bitumutil.Amount
		spendLimit  bitumutil.Amount
		minConf     int32
		ticketAddr  bitumutil.Address
		account     uint32
		numTickets  int
		poolAddress bitumutil.Address
		poolFees    float64
		expiry      int32
		txFee       bitumutil.Amount
		ticketFee   bitumutil.Amount
...
func (w *Wallet) Consolidate(inputs int, account uint32,
	address bitumutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) CreateMultisigTx(account uint32, amount bitumutil.Amount, pubkeys []*bitumutil.AddressSecpPubKey, nrequired int8, minconf int32) (*CreatedTx, bitumutil.Address, []byte, error) {
...
// tickets.
func (w *Wallet) PurchaseTickets(minBalance, spendLimit bitumutil.Amount, minConf int32, ticketAddr bitumutil.Address, account uint32, numTickets int, poolAddress bitumutil.Address,
	poolFees float64, expiry int32, txFee bitumutil.Amount, ticketFee bitumutil.Amount) ([]*chainhash.Hash, error) {
...
// spending to it in the blockchain or bitumd mempool), the next chained address
// is returned.
func (w *Wallet) CurrentAddress(account uint32) (bitumutil.Address, error) {
...
func (w *Wallet) PubKeyForAddress(a bitumutil.Address) (chainec.PublicKey, error) {
...
func (w *Wallet) SignMessage(msg string, addr bitumutil.Address) (sig []byte, err error) {
...
func VerifyMessage(msg string, addr bitumutil.Address, sig []byte) (bool, error) {
	...
	recoveredAddr, err := bitumutil.NewAddressSecpPubKey(serializedPK, addr.Net())
	if err != nil {
		return false, errors.E(op, err)
	}
...
func (w *Wallet) HaveAddress(a bitumutil.Address) (bool, error) {
...
func (w *Wallet) AccountOfAddress(a bitumutil.Address) (uint32, error) {
...
func (w *Wallet) AddressInfo(a bitumutil.Address) (udb.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *udb.TxDetails, addrMgr *udb.Manager, syncHeight int32, net *chaincfg.Params) (sends, receives []bitumjson.ListTransactionsResult) {
	addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
	...
	txTypeStr := bitumjson.LTTTRegular
	switch details.TxType {
	case stake.TxTypeSStx:
		txTypeStr = bitumjson.LTTTTicket
	case stake.TxTypeSSGen:
		txTypeStr = bitumjson.LTTTVote
	case stake.TxTypeSSRtx:
		txTypeStr = bitumjson.LTTTRevocation
	}

	// Fee can only be determined if every input is a debit.
	var feeF64 float64
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal bitumutil.Amount
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := bitumutil.Amount(output.Value).ToCoin()
		result := bitumjson.ListTransactionsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   BlockIndex
			//
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32) ([]bitumjson.ListTransactionsResult, error) {
	const op errors.Op = "wallet.ListSinceBlock"
	txList := []bitumjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]bitumjson.ListTransactionsResult, error) {
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]bitumjson.ListTransactionsResult, error) {
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*bitumutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
// replies.
func (w *Wallet) ListAllTransactions() ([]bitumjson.ListTransactionsResult, error) {
...
// transaction.
func (w *Wallet) ListTransactionDetails(txHash *chainhash.Hash) ([]bitumjson.ListTransactionsResult, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		txList = make([]bitumjson.ListTransactionsResult, 0, len(sends)+len(receives))
		txList = append(txList, receives...)
		txList = append(txList, sends...)
		return nil
	})
...
func (w *Wallet) GetTicketInfoPrecise(chainClient *bitumrpcclient.Client, hash *chainhash.Hash) (*TicketSummary, *wire.BlockHeader, error) {
...
func (w *Wallet) GetTicketsPrecise(f func([]*TicketSummary, *wire.BlockHeader) (bool, error), chainClient *bitumrpcclient.Client, startBlock, endBlock *BlockIdentifier) error {
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*bitumutil.Amount)
...
func (w *Wallet) ListUnspent(minconf, maxconf int32, addresses map[string]struct{}) ([]*bitumjson.ListUnspentResult, error) {
	const op errors.Op = "wallet.ListUnspent"
	var results []*bitumjson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		scSwitch:
			...
			result := &bitumjson.ListUnspentResult{
...
func (w *Wallet) DumpWIFPrivateKey(addr bitumutil.Address) (string, error) {
	...
	wif, err := bitumutil.NewWIF(privKey, w.chainParams, bitumec.SignatureType(privKey.GetType()))
	if err != nil {
		return "", errors.E(op, err)
	}
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *bitumutil.WIF) (string, error) {
	...
	if n, err := w.NetworkBackend(); err == nil {
		err := n.LoadTxFilter(context.TODO(), false, []bitumutil.Address{addr}, nil)
		if err != nil {
			return "", errors.E(op, err)
		}
	}
...
// a P2SH address.
func (w *Wallet) RedeemScriptCopy(addr bitumutil.Address) ([]byte, error) {
...
type StakeInfoData struct {
	BlockHeight  int64
	TotalSubsidy bitumutil.Amount
	Sdiff        bitumutil.Amount
		...
		for it.Next() {
			...
			if it.SpenderHash != (chainhash.Hash{}) {
				...
				case isVote(spender):
					...
					// earned by fees.
					res.TotalSubsidy += bitumutil.Amount(spender.TxIn[0].ValueIn)
...
func (w *Wallet) StakeInfoPrecise(chainClient *bitumrpcclient.Client) (*StakeInfoData, error) {
	const op errors.Op = "wallet.StakeInfoPrecise"
	// This is only needed for the total count and can be optimized.
	mempoolTicketsFuture := chainClient.GetRawMempoolAsync(bitumjson.GRMTickets)
...
func (w *Wallet) LockedOutpoints() []bitumjson.TransactionInput {
	locked := make([]bitumjson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = bitumjson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) SortedActivePaymentAddresses() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr bitumutil.Address) error {
...
func ticketMatured(params *chaincfg.Params, txHeight, curHeight int32) bool {
	// bitumd has an off-by-one in the calculation of the ticket
...
// returning the total amount of bitum received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(addr bitumutil.Address, minConf int32) (bitumutil.Amount, error) {
	const op errors.Op = "wallet.TotalReceivedForAddr"
	var amount bitumutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType, additionalPrevScripts map[wire.OutPoint][]byte,
	additionalKeysByAddress map[string]*bitumutil.WIF, p2shRedeemScriptsByAddress map[string][]byte) ([]SignatureError, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr bitumutil.Address) (
			...
			getScript := txscript.ScriptClosure(func(
				addr bitumutil.Address) ([]byte, error) {
			...
			if (hashType&txscript.SigHashSingle) !=
				...
				ecType := bitumec.STEcdsaSecp256k1
...
// address.
func (w *Wallet) CreateSignature(tx *wire.MsgTx, idx uint32, addr bitumutil.Address, hashType txscript.SigHashType, prevPkScript []byte) (sig, pubkey []byte, err error) {
...
func Open(cfg *Config) (*Wallet, error) {
	...
	// Amounts
	w.ticketFeeIncrement, err = bitumutil.NewAmount(cfg.TicketFee)
	if err != nil {
		return nil, errors.E(op, err)
	}
	w.relayFee, err = bitumutil.NewAmount(cfg.RelayFee)
	if err != nil {
		return nil, errors.E(op, err)
	}
