...
type InputSource func(target bitumutil.Amount) (detail *InputDetail, err error)
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb bitumutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, secrets SecretsSource) error {
	...
	for i := range inputs {
		...
		script, err := txscript.SignTxOutput(chainParams, tx, i,
			...
			sigScript, bitumec.STEcdsaSecp256k1)
		if err != nil {
			return err
		}
