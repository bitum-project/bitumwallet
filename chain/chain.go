...
// RPCClient represents a persistent client connection to a bitum RPC server
// for information regarding the current best block chain.
type RPCClient struct {
	*bitumrpcclient.Client
	connConfig  *bitumrpcclient.ConnConfig // Work around unexported field
...
// operate on the same bitum network as described by the passed chain
// parameters, the connection will be disconnected.
// Deprecated: use NewRPCClientConfig
func NewRPCClient(chainParams *chaincfg.Params, connect, user, pass string, certs []byte,
	disableTLS bool) (*RPCClient, error) {
	return NewRPCClientConfig(chainParams, &bitumrpcclient.ConnConfig{
		Host:                 connect,
		Endpoint:             "ws",
		User:                 user,
		Pass:                 pass,
...
func NewRPCClientConfig(chainParams *chaincfg.Params, connConfig *bitumrpcclient.ConnConfig) (*RPCClient, error) {
	client := &RPCClient{
	...
	ntfnCallbacks := &bitumrpcclient.NotificationHandlers{
	...
	rpcClient, err := bitumrpcclient.New(client.connConfig, ntfnCallbacks)
	if err != nil {
		return nil, err
	}
...
func (c *RPCClient) Start(ctx context.Context, retry bool) (err error) {
	...
	if err != nil {
		return errors.E(op, errors.E(errors.Op("bitumd.jsonrpc.getcurrentnet"), err))
	}
	...
	if err == nil {
		versionResult := versions["bitumdjsonrpcapi"]
		serverAPI = semver{
...
// bitumrpcclient callbacks, which isn't very Go-like and doesn't allow
...
// bitum RPC server.  This channel must be continually read or the process
...
out:
	for {
		...
		case <-pingChan:
			...
			// be due to bitumd processing websocket requests synchronously (see
...
// POSTClient creates the equivalent HTTP POST bitumrpcclient.Client.
func (c *RPCClient) POSTClient() (*bitumrpcclient.Client, error) {
	configCopy := *c.connConfig
	configCopy.HTTPPostMode = true
	client, err := bitumrpcclient.New(&configCopy, nil)
	if err != nil {
