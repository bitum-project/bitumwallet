...
import (
	...
	"github.com/bitum-project/bitumwallet/chain"
	"github.com/bitum-project/bitumwallet/errors"
	"github.com/bitum-project/bitumwallet/internal/helpers"
	"github.com/bitum-project/bitumwallet/p2p"
	ver "github.com/bitum-project/bitumwallet/version"
	"github.com/bitum-project/bitumwallet/wallet"
...
var handlers = map[string]handler{
	...
	// Reference methods which can't be implemented by bitumwallet due to
...
func unimplemented(*Server, interface{}) (interface{}, error) {
	return nil, &bitumjson.RPCError{
		Code:    bitumjson.ErrRPCUnimplemented,
		Message: "Method unimplemented",
	}
}

// unsupported handles a standard bitcoind RPC request which is
// unsupported by bitumwallet due to design differences.
func unsupported(*Server, interface{}) (interface{}, error) {
	return nil, &bitumjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by bitumwallet",
	}
}
...
// context.
type lazyHandler func() (interface{}, *bitumjson.RPCError)
...
func lazyApplyHandler(s *Server, request *bitumjson.Request) lazyHandler {
	handlerData, ok := handlers[request.Method]
	if !ok {
		return func() (interface{}, *bitumjson.RPCError) {
			...
			if err != nil {
				return nil, rpcErrorf(bitumjson.ErrRPCClientNotConnected, "RPC passthrough requires bitumd RPC synchronization")
			}
	...
	return func() (interface{}, *bitumjson.RPCError) {
		cmd, err := bitumjson.UnmarshalCmd(request)
		if err != nil {
			return nil, bitumjson.ErrRPCInvalidRequest
		}
...
func makeResponse(id, result interface{}, err error) bitumjson.Response {
	idPtr := idPointer(id)
	if err != nil {
		return bitumjson.Response{
			ID:    idPtr,
...
func accountAddressIndex(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.AccountAddressIndexCmd)
	...
	case udb.InternalBranch:
		return intChild, nil
	default:
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "invalid branch %v", cmd.Branch)
	}
}
...
func accountSyncAddressIndex(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.AccountSyncAddressIndexCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if index >= hdkeychain.HardenedKeyStart {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter,
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*bitumutil.AddressSecpPubKey, len(keys))
	...
	for i, a := range keys {
		...
		switch addr := a.(type) {
		case *bitumutil.AddressSecpPubKey:
		...
		default:
			...
			if bitumec.SignatureType(pubKey.GetType()) != bitumec.STEcdsaSecp256k1 {
				return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter,
					"only secp256k1 pubkeys are currently supported")
			}
			pubKeyAddr, err := bitumutil.NewAddressSecpPubKey(
				pubKey.Serialize(), w.ChainParams())
			if err != nil {
				return nil, rpcError(bitumjson.ErrRPCInvalidAddressOrKey, err)
			}
...
func addMultiSigAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.AddMultisigAddressCmd)
	...
	secp256k1Addrs := make([]bitumutil.Address, len(cmd.Keys))
	...
	err = n.LoadTxFilter(context.TODO(), false, []bitumutil.Address{p2shAddr}, nil)
	if err != nil {
		return nil, err
	}
...
func addTicket(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.AddTicketCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if err != nil {
		return nil, rpcError(bitumjson.ErrRPCDeserialization, err)
	}
...
func consolidate(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ConsolidateCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	var changeAddr bitumutil.Address
	if cmd.Address != nil {
		if *cmd.Address != "" {
...
func createMultiSig(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.CreateMultisigCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	address, err := bitumutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
		return nil, err
	}

	return bitumjson.CreateMultiSigResult{
...
func dumpPrivKey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.DumpPrivKeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func generateVote(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GenerateVoteCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if err != nil {
		return nil, rpcError(bitumjson.ErrRPCDecodeHexString, err)
	}
	...
	resp := &bitumjson.GenerateVoteResult{
		Hex: b.String(),
	}
	return resp, nil
}
...
func getAddressesByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetAddressesByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getBalance(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetBalanceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if minConf < 0 {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "minconf must be non-negative")
	}
	...
	result := bitumjson.GetBalanceResult{
	...
	if accountName == "*" {
		...
		var (
			totImmatureCoinbase bitumutil.Amount
			totImmatureStakegen bitumutil.Amount
			totLocked           bitumutil.Amount
			totSpendable        bitumutil.Amount
			totUnconfirmed      bitumutil.Amount
			totVotingAuthority  bitumutil.Amount
			cumTot              bitumutil.Amount
		)

		balancesLen := uint32(len(balances))
		result.Balances = make([]bitumjson.GetAccountBalanceResult, balancesLen)
		...
		for _, bal := range balances {
			...
			if err != nil {
				...
				if errors.Is(errors.NotExist, err) {
					return nil, rpcError(bitumjson.ErrRPCInternal.Code, err)
				}
				return nil, err
			}
			...
			json := bitumjson.GetAccountBalanceResult{
...
func getBestBlock(s *Server, icmd interface{}) (interface{}, error) {
	...
	result := &bitumjson.GetBestBlockResult{
		Hash:   hash.String(),
		Height: int64(height),
	}
	return result, nil
...
func getInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	var spendableBalance bitumutil.Amount
	for _, balance := range balances {
		spendableBalance += balance.Spendable
	}

	info := &bitumjson.InfoWalletResult{
		Version:         ver.Integer,
...
func decodeAddress(s string, params *chaincfg.Params) (bitumutil.Address, error) {
	...
	if len(s) == 66 || len(s) == 130 {
		...
		pubKeyAddr, err := bitumutil.NewAddressSecpPubKey(pubKeyBytes,
			params)
		if err != nil {
			return nil, err
		}
	...
	addr, err := bitumutil.DecodeAddress(s)
	if err != nil {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidAddressOrKey,
...
func getAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
// appear in the blockchain, or any tx that has arrived in the bitumd mempool).
// If the most recently-requested address has been used, a new address (the
// next chained address in the keypool) is used.  This can fail if the keypool
// runs out (and will return bitumjson.ErrRPCWalletKeypoolRanOut if that happens).
func getAccountAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetAccountAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getUnconfirmedBalance(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetUnconfirmedBalanceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func importPrivKey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ImportPrivKeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	wif, err := bitumutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidAddressOrKey, "WIF decode failed: %v", err)
	}
	if !wif.IsForNet(w.ChainParams()) {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidAddressOrKey, "key is not intended for %s", w.ChainParams().Name)
	}
...
func importScript(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ImportScriptCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if len(rs) == 0 {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "empty script")
	}
...
// keypoolRefill handles the keypoolrefill command.  bitumwallet generates
...
func createNewAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.CreateNewAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if err != nil {
		if errors.Is(errors.Locked, err) {
			return nil, rpcErrorf(bitumjson.ErrRPCWalletUnlockNeeded, "creating new accounts requires an unlocked wallet")
		}
		return nil, err
	}
	return nil, nil
...
func renameAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.RenameAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getMultisigOutInfo(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetMultisigOutInfoCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	result := &bitumjson.GetMultisigOutInfoResult{
...
func getNewAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetNewAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if cmd.GapPolicy != nil {
		...
		default:
			return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "unknown gap policy %q", *cmd.GapPolicy)
		}
	}
...
func getRawChangeAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetRawChangeAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getReceivedByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetReceivedByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getReceivedByAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetReceivedByAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getMasterPubkey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetMasterPubkeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getStakeInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	resp := &bitumjson.GetStakeInfoResult{
...
func getTickets(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetTicketsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	return &bitumjson.GetTicketsResult{Hashes: ticketHashStrs}, nil
}

// getTransaction handles a gettransaction request by returning details about
// a single transaction saved by wallet.
func getTransaction(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.GetTransactionCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if errors.Is(errors.NotExist, err) {
		return nil, rpcErrorf(bitumjson.ErrRPCNoTxInfo, "no information for transaction")
	} else if err != nil {
		return nil, err
	}
	...
	ret := bitumjson.GetTransactionResult{
		TxID:            cmd.Txid,
		Hex:             b.String(),
	...
	var (
		debitTotal  bitumutil.Amount
		creditTotal bitumutil.Amount
	...
	if len(txd.Debits) == len(txd.MsgTx.TxIn) {
		var outputTotal bitumutil.Amount
		for _, output := range txd.MsgTx.TxOut {
			outputTotal += bitumutil.Amount(output.Value)
		}
	...
	ret.Details = make([]bitumjson.GetTransactionDetailsResult, len(details))
	for i, d := range details {
		ret.Details[i] = bitumjson.GetTransactionDetailsResult{
			Account:           d.Account,
			Address:           d.Address,
			Amount:            d.Amount,
			Category:          d.Category,
...
func getVoteChoices(s *Server, icmd interface{}) (interface{}, error) {
	...
	resp := &bitumjson.GetVoteChoicesResult{
		Version: version,
		Choices: make([]bitumjson.VoteChoice, len(agendas)),
	}
	...
	for i := range choices {
		resp.Choices[i] = bitumjson.VoteChoice{
...
func help(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.HelpCmd)
	// TODO: The "help" RPC should use a HTTP POST client when calling down to
	// bitumd for additional help methods.  This avoids including websocket-only
	...
	return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "no help for method %q", *cmd.Command)
}

// listAccounts handles a listaccounts request by returning a map of account
// names to their balances.
func listAccounts(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ListAccountsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listReceivedByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ListReceivedByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	jsonResults := make([]bitumjson.ListReceivedByAccountResult, 0, len(results))
	for _, result := range results {
		jsonResults = append(jsonResults, bitumjson.ListReceivedByAccountResult{
...
func listReceivedByAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ListReceivedByAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	ret := make([]bitumjson.ListReceivedByAddressResult, numAddresses)
	idx := 0
	for address, addrData := range allAddrData {
		ret[idx] = bitumjson.ListReceivedByAddressResult{
			Address:       address,
...
func listSinceBlock(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ListSinceBlockCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if targetConf < 1 {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "target_confirmations must be positive")
	}
	...
	res := &bitumjson.ListSinceBlockResult{
...
func listScripts(s *Server, icmd interface{}) (interface{}, error) {
	...
	listScriptsResultSIs := make([]bitumjson.ScriptInfo, len(redeemScripts))
	for i, redeemScript := range redeemScripts {
		p2shAddr, err := bitumutil.NewAddressScriptHash(redeemScript,
			w.ChainParams())
		if err != nil {
			return nil, err
		}
		listScriptsResultSIs[i] = bitumjson.ScriptInfo{
			Hash160:      hex.EncodeToString(p2shAddr.Hash160()[:]),
			Address:      p2shAddr.EncodeAddress(),
			RedeemScript: hex.EncodeToString(redeemScript),
		}
	}
	return &bitumjson.ListScriptsResult{Scripts: listScriptsResultSIs}, nil
}

// listTransactions handles a listtransactions request by returning an
// array of maps with details of sent and recevied wallet transactions.
func listTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ListTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listAddressTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ListAddressTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listAllTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ListAllTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listUnspent(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ListUnspentCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func lockUnspent(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.LockUnspentCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func purchaseTicket(s *Server, icmd interface{}) (interface{}, error) {
	...
	cmd := icmd.(*bitumjson.PurchaseTicketCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	spendLimit, err := bitumutil.NewAmount(cmd.SpendLimit)
	if err != nil {
		return nil, rpcError(bitumjson.ErrRPCInvalidParameter, err)
	}
	if spendLimit < 0 {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "negative spend limit")
	}
	...
	if cmd.MinConf != nil {
		minConf = int32(*cmd.MinConf)
		if minConf < 0 {
			return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "negative minconf")
		}
	}

	// Set ticket address if specified.
	var ticketAddr bitumutil.Address
	...
	var poolAddr bitumutil.Address
	var poolFee float64
	if cmd.PoolAddress != nil {
		if *cmd.PoolAddress != "" {
			...
			// the pool after.
			if cmd.PoolFees == nil {
				return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "pool address set without pool fee")
			}
			poolFee = *cmd.PoolFees
			if !txrules.ValidPoolFeeRate(poolFee) {
				return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "pool fee percentage %v", poolFee)
			}
		}
	}
	...
	if cmd.TicketFee != nil {
		ticketFee, err = bitumutil.NewAmount(*cmd.TicketFee)
		if err != nil {
			return nil, rpcError(bitumjson.ErrRPCInvalidParameter, err)
		}
	}
...
// and amounts.
func makeOutputs(pairs map[string]bitumutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
...
// All errors are returned in bitumjson.RPCError format
func sendPairs(w *wallet.Wallet, amounts map[string]bitumutil.Amount, account uint32, minconf int32) (string, error) {
	...
	if err != nil {
		...
		if errors.Is(errors.InsufficientBalance, err) {
			return "", rpcError(bitumjson.ErrRPCWalletInsufficientFunds, err)
		}
		return "", err
	}
...
func redeemMultiSigOut(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.RedeemMultiSigOutCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	srtc := &bitumjson.SignRawTransactionCmd{
		RawTx:    b.String(),
		Inputs:   &rtis,
		PrivKeys: &[]string{},
		Flags:    &sigHashAll,
	...
	srtTyped := signedTxResult.(bitumjson.SignRawTransactionResult)
	return bitumjson.RedeemMultiSigOutResult(srtTyped), nil
}
...
func redeemMultiSigOuts(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.RedeemMultiSigOutsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	p2shAddr, ok := addr.(*bitumutil.AddressScriptHash)
	if !ok {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "address is not P2SH")
	}
	...
	itr := uint32(0)
	rmsoResults := make([]bitumjson.RedeemMultiSigOutResult, len(msos))
	for i, mso := range msos {
		if itr > max {
			break
		}

		rmsoRequest := &bitumjson.RedeemMultiSigOutCmd{
		...
		redeemResultTyped := redeemResult.(bitumjson.RedeemMultiSigOutResult)
		rmsoResults[i] = redeemResultTyped

		itr++
	}

	return bitumjson.RedeemMultiSigOutsResult{Results: rmsoResults}, nil
}
...
func rescanWallet(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.RescanWalletCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func stakePoolUserInfo(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.StakePoolUserInfoCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	userAddr, err := bitumutil.DecodeAddress(cmd.User)
	if err != nil {
		return nil, err
	}
	...
	resp := new(bitumjson.StakePoolUserInfoResult)
	resp.Tickets = make([]bitumjson.PoolUserTicket, 0, len(spui.Tickets))
	resp.InvalidTickets = make([]string, 0, len(spui.InvalidTickets))
	for _, ticket := range spui.Tickets {
		var ticketRes bitumjson.PoolUserTicket
...
func ticketsForAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.TicketsForAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	addr, err := bitumutil.DecodeAddress(cmd.Address)
	if err != nil {
		return nil, err
	}
	...
	return bitumjson.TicketsForAddressResult{Tickets: ticketHashStrs}, nil
}
...
func sendFrom(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SendFromCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if !isNilOrEmpty(cmd.Comment) || !isNilOrEmpty(cmd.CommentTo) {
		return nil, rpcErrorf(bitumjson.ErrRPCUnimplemented, "transaction comments are unsupported")
	}
	...
	if cmd.Amount < 0 {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "negative amount")
	}
	minConf := int32(*cmd.MinConf)
	if minConf < 0 {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "negative minconf")
	...
	amt, err := bitumutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(bitumjson.ErrRPCInvalidParameter, err)
	}
	pairs := map[string]bitumutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SendManyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	// Recreate address/amount pairs, using bitumutil.Amount.
	pairs := make(map[string]bitumutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := bitumutil.NewAmount(v)
		if err != nil {
			return nil, rpcError(bitumjson.ErrRPCInvalidParameter, err)
		}
		pairs[k] = amt
...
func sendToAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SendToAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func sendToMultiSig(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SendToMultiSigCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	account := uint32(udb.DefaultAccountNum)
	amount, err := bitumutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(bitumjson.ErrRPCInvalidParameter, err)
	}
	nrequired := int8(*cmd.NRequired)
	minconf := int32(*cmd.MinConf)
	pubkeys := make([]*bitumutil.AddressSecpPubKey, len(cmd.Pubkeys))
	...
	result := &bitumjson.SendToMultiSigResult{
...
func setTicketFee(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SetTicketFeeCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if cmd.Fee < 0 {
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "negative fee")
	}

	incr, err := bitumutil.NewAmount(cmd.Fee)
	if err != nil {
		return nil, rpcError(bitumjson.ErrRPCInvalidParameter, err)
	}
...
func setTxFee(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SetTxFeeCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	relayFee, err := bitumutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(bitumjson.ErrRPCInvalidParameter, err)
	}
...
func setVoteChoice(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SetVoteChoiceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func signMessage(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SignMessageCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func signRawTransaction(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SignRawTransactionCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	case "ssrtx": // Special case of SigHashAll
		hashType = txscript.SigHashAll
	default:
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "invalid sighash flag")
	}

	// TODO: really we probably should look these up with bitumd anyway to
	// make sure that they match the blockchain if present.
	inputs := make(map[wire.OutPoint][]byte)
	scripts := make(map[string][]byte)
	var cmdInputs []bitumjson.RawTxInput
	if cmd.Inputs != nil {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := bitumutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, err
			}
	...
	// querying bitumd with getrawtransaction. We queue up a bunch of async
	...
	var keys map[string]*bitumutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*bitumutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := bitumutil.DecodeWIF(key)
			if err != nil {
				return nil, rpcError(bitumjson.ErrRPCDeserialization, err)
			}

			if !wif.IsForNet(w.ChainParams()) {
				return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "key intended for different network")
			}

			var addr bitumutil.Address
			switch wif.DSA() {
			case bitumec.STEcdsaSecp256k1:
				addr, err = bitumutil.NewAddressSecpPubKey(wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
				}
			case bitumec.STEd25519:
				addr, err = bitumutil.NewAddressEdwardsPubKey(
					wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
				}
			case bitumec.STSchnorrSecp256k1:
				addr, err = bitumutil.NewAddressSecSchnorrPubKey(
					wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
	...
	for outPoint, resp := range requested {
		...
		if err != nil {
			return nil, errors.E(errors.Op("bitumd.jsonrpc.gettxout"), err)
		}
	...
	signErrors := make([]bitumjson.SignRawTransactionError, 0, len(signErrs))
	for _, e := range signErrs {
		input := tx.TxIn[e.InputIndex]
		signErrors = append(signErrors, bitumjson.SignRawTransactionError{
	...
	return bitumjson.SignRawTransactionResult{
		Hex:      b.String(),
...
func signRawTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SignRawTransactionsCmd)

	// Sign each transaction sequentially and record the results.
	// Error out if we meet some unexpected failure.
	results := make([]bitumjson.SignRawTransactionResult, len(cmd.RawTxs))
	...
	if *cmd.Send {
		...
		for i, result := range results {
			if result.Complete {
				...
				st := bitumjson.SignedTransaction{
...
func startAutoBuyer(s *Server, icmd interface{}) (interface{}, error) {
	...
	if cmd.VotingAddress != nil {
		var votingAddress bitumutil.Address
	...
	var poolAddress bitumutil.Address
	if cmd.PoolAddress != nil {
		if *cmd.PoolAddress != "" {
	...
	case poolFees == 0 && poolAddress != nil:
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "pooladdress set without poolfees")
	case poolFees != 0 && poolAddress == nil:
		return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "poolfees set without pooladdress")
	case poolFees != 0 && poolAddress != nil:
		if !txrules.ValidPoolFeeRate(poolFees) {
			return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "invalid poolfees %v", poolFees)
		}
	}
...
func makeScriptChangeSource(address string, version uint16) (*scriptChangeSource, error) {
	destinationAddress, err := bitumutil.DecodeAddress(address)
	if err != nil {
		return nil, err
	}
...
func sweepAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.SweepAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if cmd.FeePerKb != nil {
		var err error
		feePerKb, err = bitumutil.NewAmount(*cmd.FeePerKb)
		if err != nil {
			return nil, rpcError(bitumjson.ErrRPCInvalidParameter, err)
		}
	}
	...
	if err != nil {
		if errors.Is(errors.InsufficientBalance, err) {
			return nil, rpcError(bitumjson.ErrRPCWalletInsufficientFunds, err)
		}
		return nil, err
	}
	...
	res := &bitumjson.SweepAccountResult{
...
func validateAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.ValidateAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	result := bitumjson.ValidateAddressWalletResult{}
	addr, err := decodeAddress(cmd.Address, w.ChainParams())
	if err != nil {
...
func verifyMessage(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.VerifyMessageCmd)
	...
	switch a := addr.(type) {
	case *bitumutil.AddressSecpPubKey:
	case *bitumutil.AddressPubKeyHash:
		if a.DSA(a.Net()) != bitumec.STEcdsaSecp256k1 {
			goto WrongAddrKind
		}
	default:
		goto WrongAddrKind
...
WrongAddrKind:
	return nil, rpcErrorf(bitumjson.ErrRPCInvalidParameter, "address must be secp256k1 P2PK or P2PKH")
}
...
func version(s *Server, icmd interface{}) (interface{}, error) {
	var resp map[string]bitumjson.VersionResult
	n, _ := s.walletLoader.NetworkBackend()
	chainClient, err := chain.RPCClientFromBackend(n)
	if err == nil {
		var err error
		...
		resp = make(map[string]bitumjson.VersionResult)
	}

	resp["bitumwalletjsonrpcapi"] = bitumjson.VersionResult{
...
func walletInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	return &bitumjson.WalletInfoResult{
...
func walletPassphrase(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.WalletPassphraseCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func walletPassphraseChange(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bitumjson.WalletPassphraseChangeCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if err != nil {
		if errors.Is(errors.Passphrase, err) {
			return nil, rpcErrorf(bitumjson.ErrRPCWalletPassphraseIncorrect, "incorrect passphrase")
		}
		return nil, err
	}
	return nil, nil
...
func decodeHexStr(hexStr string) ([]byte, error) {
	...
	if err != nil {
		return nil, rpcErrorf(bitumjson.ErrRPCDecodeHexString, "hex string decode failed: %v", err)
	}
	return decoded, nil
}
