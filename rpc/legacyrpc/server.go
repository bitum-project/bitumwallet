...
// method.  This may be a request that is handled directly by bitumwallet, or
// a chain server request that is handled by passing the request down to bitumd.
//
...
func (s *Server) handlerClosure(ctx context.Context, request *bitumjson.Request) lazyHandler {
...
// but this is only implemented here to work around an oddity with bitumjson,
...
func (s *Server) invalidAuth(req *bitumjson.Request) bool {
	cmd, err := bitumjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*bitumjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			if err != nil {
				...
				resp := makeResponse(req.ID, nil,
					bitumjson.ErrRPCInvalidRequest)
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := bitumjson.MarshalResponse(req.Jsonrpc, req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := bitumjson.MarshalResponse(req.Jsonrpc, req.ID, nil, bitumjson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response to client %s: %v",
				r.RemoteAddr, err)
	...
	var res interface{}
	var jsonErr *bitumjson.RPCError
	var stop bool
	switch req.Method {
	case "authenticate":
	...
	case "stop":
		...
		stop = true
		res = "bitumwallet stopping"
	default:
		res, jsonErr = s.handlerClosure(ctx, &req)()
	}

	// Marshal and send.
	mresp, err := bitumjson.MarshalResponse(req.Jsonrpc, req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response to client %s: %v",
			r.RemoteAddr, err)
