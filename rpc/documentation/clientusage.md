...
var certificateFile = filepath.Join(bitumutil.AppDataDir("bitumwallet", false), "rpc.cert")
...
func main() {
	...
	fmt.Println("Spendable balance: ", bitumutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/bitumwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/bitumwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by bitumwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.bitumwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\bitumwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by bitumwallet).
...
if (os.platform() == 'win32') {
  ...
  certPath = path.join(process.env.HOME, '.bitumwallet', 'rpc.cert');
}
...
```bash
$ protoc -I /path/to/bitum/bitumwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    if platform.system() == 'Windows':
        ...
        cert_file_path = os.path.join(os.environ['HOME'], '.bitumwallet', 'rpc.cert')
