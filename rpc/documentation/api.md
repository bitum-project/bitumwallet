...
the management of the wallet and its connection to the Bitum network.  It has
...
dynamically start the bitumd RPC client.  This RPC client is used for wallet
syncing and publishing transactions to the Bitum network.
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default bitumd RPC port of the
  active Bitum network.
...
- `string public_key`: The public key encoded as a string in the Bitum encoding
  format.
...
the Bitum network.  If the transaction spends any of the wallet's unspent
...
notifications passed from the JSON RPC of bitumd. Right now, wallet block processing
