...
import (
	...
	"github.com/bitum-project/bitumwallet/version"
...
const (
	defaultCAFilename          = "bitumd.cert"
	defaultConfigFilename      = "bitumwallet.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "bitumwallet.log"
	...
	defaultMaxFee                    bitumutil.Amount = 1e6
	defaultMinFee                    bitumutil.Amount = 1e5
...
var (
	bitumdDefaultCAFile  = filepath.Join(bitumutil.AppDataDir("bitumd", false), "rpc.cert")
	defaultAppDataDir  = bitumutil.AppDataDir("bitumwallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of bitumd RPC server to connect to"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with bitumd"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	DcrdUsername     string                  `long:"bitumdusername" description:"Username for bitumd authentication"`
	DcrdPassword     string                  `long:"bitumdpassword" default-mask:"-" description:"Password for bitumd authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy JSON-RPC and bitumd authentication (if bitumdusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy JSON-RPC and bitumd authentication (if bitumdpassword is unset)"`
...
//
// The above results in bitumwallet functioning properly without any config
...
func loadConfig(ctx context.Context) (*config, []string, error) {
	...
	if cfg.DisableClientTLS {
		...
		// If CAFile is unset, choose either the copy or local bitumd cert.
		if !cfg.CAFile.ExplicitlySet() {
			cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

			// If the CA copy does not exist, check if we're connecting to
			// a local bitumd and switch to its RPC cert if it exists.
			...
			if !certExists {
				if _, ok := localhostListeners[RPCHost]; ok {
					bitumdCertExists, err := cfgutil.FileExists(
						bitumdDefaultCAFile)
					if err != nil {
						fmt.Fprintln(os.Stderr, err)
						return loadConfigError(err)
					...
					if bitumdCertExists {
						cfg.CAFile.Value = bitumdDefaultCAFile
					}
				}
			}
		}
	...
	// If the bitumd username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for bitumd and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for bitumd and wallet.
	if cfg.DcrdUsername == "" {
